...
func test(filename string) error {
	...
	if option != "parse" {
		...
		if err != nil {
			value := err.Error()
			switch value {
			case "ReferenceError: window is not defined":
			case "ReferenceError: navigator is not defined":
			case "ReferenceError: jQuery is not defined":
			case "ReferenceError: document is not defined":
			case "ReferenceError: $ is not defined":
			case "ReferenceError: require is not defined":
			case "ReferenceError: define is not defined":
			default:
				switch {
	...
	if parse {
		...
		_, err = parser.ParseFile(nil, filename, string(script), 0)
		if err != nil {
			return err
		}
	}
