...
func (runtime *_runtime) parse(source string) (*ast.Program, error) {
	return parser.ParseFile(nil, "", source, 0)
}

func mustParse(program *ast.Program, err error) *ast.Program {
	if err != nil {
		panic(err)
	}
	return program
